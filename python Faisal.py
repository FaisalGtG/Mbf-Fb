...
def Ugen():
	...
	z = random.choice([
		...
		f"Mozilla/5.0 (Linux; android 500; IPHONE 17 PROMAX) AppleWebKit/537.36 (KHTML, LikeGecko) Yandex/Versi_IPHONE 17 PROMAX",
		f"Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_5 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Mobile/13G36 [FBAN/MessengerForiOS;FBAV/90.0.0.24.70;FBBV/39954546;FBRV/0;FBDV/iPhone7,2;FBMD/iPhone;FBSN/iPhone OS;FBSV/9.3.5;FBSS/2;FBCR/VinaPhone;FBID/phone;FBLC/vi_VN;FBOP/5]"
	])
	return z
...
def logomenu():
  ...
  logo = coy(nel(f'''[bold red]  ●[bold yellow] ●[bold green] ●                                     {P2}Version {H2}1.5
    {warna2}__________                     __________________
    ___  ____/_____ ___________    ___  __ )__  ____/
    __  /_   _  __ `/  ___/  _ \   __  __  |_  /_    
    {P2}_  __/   / /_/ // /__ /  __/   _  /_/ /_  __/    
    {P2}/_/      \__,_/ \___/ \___/    /_____/ /_/       By Bang Icall
                 
                   [on #ff0000]Coded By FaisalGtG.
...
def Menu():
	...
	tree.add(f'{P2}05. Report Bug           [{H2}WhatsApp{P2}]')
	tree.add(f'{P2}00. Delete Cookie        [{M2}LogOut{P2}]')
	console.print(tree)
	print('')
	__wahyu_xd__ = input(f'{P}[{warna}?{P}] Choice: ')
	if   __wahyu_xd__ in ['01','1']:
	  panel2 = nel(f'{P2}Masukkan ID Target Kamu, Gunakan Koma Sebagai Pemisah\nEx:{H2} 123456,123456{P2}',width=60,style='#ff00f1')
	  tree = Tree(panel2)
	  console.print(tree)
	  idt = input(f'{P}╰─>{H} ')
	  print('')
	  coy(nel(f'{P2}      Tekan {H2} Ctrl + C {P2}Jika Ingin Berhenti Dump ID',subtitle=f'{P2}╭──',subtitle_align='left',width=60,style='#ff14cf'))
